...
class StatisticalAnalyzer:
    ...
    def bootstrap_confidence_interval(
        ...
        means = [np.mean(np.random.choice(arr, size=len(arr), replace=True)) for _ in range(num_resamples)]
    ...
    @staticmethod
    def compare_distributions(baseline_scores: List[float], compressed_scores: List[float]) -> Dict[str, float]:
        ...
        stat, p_val = stats.mannwhitneyu(baseline_scores, compressed_scores, alternative="two-sided")
        return {
