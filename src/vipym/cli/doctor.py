...
def run_doctor_checks() -> bool:
    ...
    table.add_row("Python (>= 3.11)", "✓ OK" if py_ok else "✗ FAIL", f"v{py_ver} ({platform.python_implementation()})")
    if not py_ok:
        all_passed = False
    ...
    table.add_row("Docker Sandbox", "✓ OK" if docker_ok else "⚠ WARN", f"Binary: {docker_bin or 'Not Found (Subprocess fallback)'}")

    # 6. Disk Space
    total, used, free = shutil.disk_usage(".")
    free_gb = free / (1024**3)
    disk_ok = free_gb > 10.0
    table.add_row("Disk Space", "✓ OK" if disk_ok else "⚠ WARN", f"{free_gb:.1f} GB free on working drive")

    # 7. AWS Credentials Check
    aws_key = os.environ.get("AWS_ACCESS_KEY_ID")
    table.add_row("AWS Credentials", "✓ OK" if aws_key else "ℹ INFO", "Found in env" if aws_key else "Not configured (Local execution mode)")

    # 8. Hugging Face Auth Check
    hf_token = os.environ.get("HF_TOKEN") or os.environ.get("HUGGING_FACE_HUB_TOKEN")
    table.add_row("Hugging Face Auth", "✓ OK" if hf_token else "ℹ INFO", "Token present" if hf_token else "No token (Public models only)")
