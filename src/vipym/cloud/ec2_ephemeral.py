...
class EphemeralNodeSpec(BaseModel):
    ...
    max_spot_price_usd: Optional[float] = None
...
class EphemeralEC2Manager:
    ...
    def launch_ephemeral_experiment_node(self, spec: EphemeralNodeSpec, user_data_script: str) -> str:
        """Launch an ephemeral GPU EC2 instance with an auto-poweroff watchdog in user-data."""
        logger.info(f"Launching ephemeral node: type={spec.instance_type}, spot={spec.spot_instance}")
